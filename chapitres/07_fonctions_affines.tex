...
Une entreprise de transport express propose deux formules tarifaires pour la livraison de colis en zone régionale :
\begin{itemize}
    \item \textbf{Tarif Éco (Formule Linéaire)} : Aucun abonnement, facturation de \textbf{2 \euro{}} par kilomètre parcouru $x$.
    \item \textbf{Tarif Pro (Formule Affine)} : Forfait fixe mensuel de prise en charge de \textbf{30 \euro{}}, puis \textbf{1 \euro{}} par kilomètre parcouru $x$.
\end{itemize}
...
\begin{tikzpicture}[scale=0.85]
    ...
    \draw[thick, ->, >=stealth, slate800] (0,0) -- (8.3,0) node[right, font=\footnotesize] {Distance $x$ (km)};
    \draw[thick, ->, >=stealth, slate800] (0,0) -- (0,6.3) node[above, font=\footnotesize] {Prix $y$ (\euro{})};
    ...
    \foreach \y/\val in {1/20, 2/40, 3/60, 4/80, 5/100, 6/120}
        \draw (0.1,\y) -- (-0.1,\y) node[left, font=\scriptsize] {\val};
        
    % Droites
    \draw[line width=1.5pt, colPrimary] (0,0) -- (4,4) -- (6,6) node[above left, font=\bfseries\footnotesize] {Tarif Éco : $f(x)=2x$};
    \draw[line width=1.5pt, colSecondary] (0,1.5) -- (6,4.5) -- (8,5.5) node[above right, font=\bfseries\footnotesize] {Tarif Pro : $g(x)=x+30$};
    
    % Point d'intersection
    \fill[amber!90!black] (3,3) circle (3.5pt);
    \draw[dashed, thick, amber!90!black] (3,0) -- (3,3) -- (0,3);
    \node[above left, font=\bfseries\footnotesize, amber!90!black] at (3,3) {Intersection $(30\text{ km} ; 60\text{ \euro{}})$};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}
    \node[draw=colPrimary, fill=slate50, rounded corners=6pt, inner sep=8pt, text width=\dimexpr\linewidth-28pt\relax] {
        ...
        \textbf{Problématique :}
        \begin{enumerate}
            \item Quelle droite passe par l'origine du repère $(0;0)$ ? Que traduit-elle sur la proportionnalité ?
            \item À partir de quelle distance parcourue le Tarif Pro devient-il plus avantageux pour le client ?
            \item Comment modéliser mathématiquement ces deux tarifs à l'aide de fonctions ?
        \end{enumerate}
    };
\end{tikzpicture}
\end{center}
...
Une \textbf{fonction} $f$ est un processus mathématique (une « machine ») qui, à chaque nombre de départ noté $x$, associe un \textbf{unique} nombre d'arrivée noté $f(x)$ (se lit « $f$ de $x$ »).
\[ x \xrightarrow{\quad f \quad} f(x) \]
\begin{itemize}
    \item $f(x)$ est l'\textbf{image} du nombre $x$ par la fonction $f$ (axe vertical des ordonnées $y$).
    \item $x$ est un \textbf{antécédent} de $f(x)$ par la fonction $f$ (axe horizontal des abscisses $x$).
\end{itemize}
\end{cadredef}
...
\begin{itemize}
    \item Elle modélise une situation de \textbf{proportionnalité}.
    \item Sa représentation graphique est une \textbf{droite passant par l'origine} du repère $(0;0)$.
    \item Le nombre $a$ est appelé le \textbf{coefficient directeur} (ou pente de la droite).
\end{itemize}
\end{cadredef}
...
\begin{itemize}
    ...
    \item $\mathbf{a}$ est le \textbf{coefficient directeur} : il indique l'inclinaison / la pente de la droite.
    \item $\mathbf{b}$ est l'\textbf{ordonnée à l'origine} : c'est la valeur où la droite coupe l'axe vertical $(y)$ en $(0;b)$.
\end{itemize}
\end{cadredef}

\begin{cadremethode}[Méthode pas à pas : Tracer la droite d'une fonction affine]
Pour tracer la droite représentant la fonction $f(x) = 2x - 3$ :
\begin{enumerate}
    \item On choisit deux valeurs de $x$ au hasard (par exemple $x = 0$ et $x = 3$).
    \item On calcule leurs images pour obtenir les coordonnées de deux points $A$ et $B$ :
    \begin{itemize}
        \item Pour $x = 0$ : $f(0) = 2 \times 0 - 3 = -3 \implies \text{Point } A(0\,;\,-3)$.
        \item Pour $x = 3$ : $f(3) = 2 \times 3 - 3 = 6 - 3 = 3 \implies \text{Point } B(3\,;\,3)$.
    \end{itemize}
...
\begin{itemize}
    \item Image : calcul sur l'axe vertical ($y$) en remplaçant $x$.
    \item Antécédent : calcul sur l'axe horizontal ($x$) en résolvant l'équation $f(x) = k$.
    \item Fonction linéaire : $f(x) = ax$ $\implies$ droite qui \textbf{passe par l'origine} $(0;0)$.
    \item Fonction affine : $f(x) = ax + b$ $\implies$ droite qui coupe l'axe vertical en $(0;b)$.
    \item Si $a > 0$, la fonction est \textbf{croissante} (la droite monte). Si $a < 0$, elle est \textbf{décroissante} (la droite descend).
\end{itemize}
\end{cadreretenu}
...
\begin{enumerate}
    \item Calculer les images de $-2$, de $0$ et de $5$ :
    \begin{itemize}
        \item $f(-2) = \pointille[4cm]$
        \item $f(0) = \pointille[4cm]$
        \item $f(5) = \pointille[4cm]$
    \end{itemize}
...
On donne la représentation graphique d'une fonction $f$ ci-dessous :

\begin{center}
\begin{tikzpicture}[scale=0.8]
    \draw[step=1cm, slate200, very thin] (-3,-2) grid (5,5);
    \draw[thick, ->, >=stealth, slate800] (-3,0) -- (5.3,0) node[right] {$x$};
    \draw[thick, ->, >=stealth, slate800] (0,-2) -- (0,5.3) node[above] {$y$};
    ...
    % Droite y = x + 1
    \draw[line width=1.5pt, colPrimary] (-3,-2) -- (4,5) node[above right] {$(d_f)$};
\end{tikzpicture}
\end{center}

Par lecture graphique (en laissant apparaître les pointillés sur le repère) :
\begin{enumerate}
    ...
    \item Donner l'ordonnée à l'origine $b$ de cette droite : $b = \pointille[2cm]$
\end{enumerate}
\end{exercice}
...
\begin{tikzpicture}[scale=0.85]
    ...
    \draw[thick, ->, >=stealth, slate800] (-3,0) -- (5.3,0) node[right] {$x$};
    \draw[thick, ->, >=stealth, slate800] (0,-3) -- (0,5.3) node[above] {$y$};
    \foreach \x in {-2,-1,1,2,3,4}
        \draw (\x,0.1) -- (\x,-0.1) node[below, font=\scriptsize] {\x};
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Sélectionner \textbf{Ajouter une fonction} $\rightarrow$ choisir \textbf{Affine} ou taper directement : \texttt{f(x) = 2x - 3}.
    ...
    \begin{itemize}
        ...
        \item Observer les deux droites. Déplacer le curseur jusqu'au point d'intersection des deux droites.
    ...
    \begin{itemize}
        ...
        \item Noter la valeur de $f(5)$ donnée par la machine : $f(5) = \pointille[2cm]$.
    \end{itemize}
\end{enumerate}
\end{tpinfo}

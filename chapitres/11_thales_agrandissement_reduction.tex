...
\begin{cadredef}[Conditions d'Application du Théorème de Thalès]
Soient deux droites sécantes en un point $A$, coupées par deux droites $(MN)$ et $(BC)$.
\begin{center}
\textbf{Si les droites $(MN)$ et $(BC)$ sont strictement parallèles, alors :}
\[ \mathbf{\frac{AM}{AB} = \frac{AN}{AC} = \frac{MN}{BC}} \quad \left(\frac{\text{Petit triangle}}{\text{Grand triangle}}\right) \]
\end{center}
\end{cadredef}

\begin{cadremethode}[Calculer une Longueur Inconnue par Produit en Croix]
\textbf{Exemple} : Dans un triangle $ABC$, $(DE) \parallel (BC)$ avec $AB = 10\text{ cm}$, $AD = 4\text{ cm}$ et $BC = 15\text{ cm}$.
\begin{enumerate}[leftmargin=1.5em, itemsep=2pt]
    \item Les droites $(DB)$ et $(EC)$ sont sécantes en $A$, et $(DE) \parallel (BC)$.
    \item D'après le théorème de Thalès : $\frac{AD}{AB} = \frac{AE}{AC} = \frac{DE}{BC} \implies \frac{4}{10} = \frac{DE}{15}$.
...
% ------------------------------------------------------------------------------
\section{Agrandissements \& Réductions (Effets sur $k$, $k^2$, $k^3$)}

\begin{cadredef}[Rapport d'Agrandissement / Réduction ($k$)]
Lorsqu'on multiplie toutes les dimensions d'une figure par un nombre strictement positif $k$ :
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
    \item Si $\mathbf{k > 1}$ : il s'agit d'un \textbf{agrandissement} (ex : échelle $2:1 \implies k = 2$).
    \item Si $\mathbf{0 < k < 1}$ : il s'agit d'une \textbf{réduction} (ex : maquette à l'échelle $1:50 \implies k = 0{,}02$).
\end{itemize}
\end{cadredef}

\begin{center}
\begin{tcolorbox}[colback=colPrimary!10, colframe=colPrimary, width=0.96\linewidth, arc=4pt, boxrule=1pt, top=4pt, bottom=4pt, left=8pt, right=8pt]
\sffamily\bfseries\normalsize\centering
Propriété Fondamentale des Échelles ($k$) :\\[2pt]
\begin{tabular}{rcccl}
$\bullet$ Les \textbf{Longueurs} & sont multipliées par & $\mathbf{k}$ & & ($\text{Périmètre}' = k \times \text{Périmètre}$) \\
$\bullet$ Les \textbf{Aires} & sont multipliées par & $\mathbf{k^2}$ & & ($\text{Aire}' = k^2 \times \text{Aire}$) \\
$\bullet$ Les \textbf{Volumes} & sont multipliés par & $\mathbf{k^3}$ & & ($\text{Volume}' = k^3 \times \text{Volume}$)
\end{tabular}
\end{tcolorbox}
\end{center}

\begin{cadreretenu}[L'Essentiel du Chapitre 11]
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
    \item Toujours vérifier l'alignement des points et le parallélisme des droites avant d'écrire les 3 rapports.
    \item Le coefficient $k = \frac{\text{longueur finale}}{\text{longueur initiale}}$ permet de passer directement d'une figure à l'autre.
    \item Pour une surface $\implies k^2$ ; pour un volume ou une contenance $\implies k^3$.
\end{itemize}
\end{cadreretenu}

...
\begin{cadreexemple}[Exemple à compléter ensemble en classe]
\begin{itemize}
...
\hfill
\begin{minipage}{0.48\linewidth}
\textbf{Exemple 2 : Décomposition de 120 (à compléter)}
\begin{center}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{r|l}
120 & \pointille[1cm] \\
...
\textbf{Exemple modèle à compléter :} Simplifier la fraction $\dfrac{84}{126}$.
\begin{itemize}
...
\begin{tpinfo}[Calculatrice Casio fx-92+ \& TI-Collège Plus]{Décomposition et Fractions}
\begin{enumerate}
    \item \textbf{Décomposition automatique sur calculatrice} :
    \begin{itemize}
        \item Sur Casio : taper le nombre (ex: $252$) $\rightarrow$ touche \texttt{EXE} $\rightarrow$ touches \texttt{SECONDE} puis \texttt{FACT}.
        \item Noter le résultat affiché pour $252$ : $252 = \pointille[4cm]$
    \end{itemize}
    \item \textbf{Fraction irréductible} :
    \begin{itemize}
        \item Entrer la fraction $\dfrac{252}{378}$ à l'aide de la touche fraction $\dfrac{\square}{\square}$ $\rightarrow$ taper \texttt{EXE}.
        \item Noter la fraction irréductible donnée par la machine : $\dfrac{252}{378} = \pointille[3cm]$
    \end{itemize}
    \item Utiliser la calculatrice pour simplifier instantanément : $\dfrac{1\,260}{2\,100} = \pointille[3cm]$
\end{enumerate}
\end{tpinfo}

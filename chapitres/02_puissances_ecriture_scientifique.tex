...
\begin{tpinfo}[Calculatrice NumWorks]{Puissances et Notation Scientifique}
\begin{enumerate}
    \item \textbf{Saisir une puissance (Application Calculs)} :
    \begin{itemize}
        \item Pour calculer $3^5$ : taper \texttt{3} $\rightarrow$ touche puissance \texttt{$x^\blacksquare$} $\rightarrow$ \texttt{5} $\rightarrow$ \texttt{EXE}.
        \item Noter le résultat affiché : $3^5 = \pointille[3cm]$
    \end{itemize}
    \item \textbf{Saisir une puissance de 10} :
    \begin{itemize}
        \item Utiliser la touche \texttt{$\times 10^x$} située en bas du clavier à côté de \texttt{Ans}.
        \item Pour entrer $4{,}2 \times 10^6$ : taper \texttt{4.2} $\rightarrow$ touche \texttt{$\times 10^x$} $\rightarrow$ \texttt{6} $\rightarrow$ \texttt{EXE}.
    \end{itemize}
    \item \textbf{Mode Scientifique} :
    \begin{itemize}
        \item Configurer la NumWorks en notation scientifique : Touche \textbf{Maison} $\rightarrow$ \textbf{Paramètres} $\rightarrow$ \textit{Format des résultats} $\rightarrow$ sélectionner \textbf{Scientifique}.
